...
class CollectPosts(object):
    ...
    def collect_page(self, page):
        ...
        for link in links:
            print("click link see more")
            link.click()
        # posts = self.browser.find_elements_by_class_name("userContentWrapper") 
        posts = self.browser.find_elements_by_xpath('//div[@data-ad-comet-preview="message"]')


        # poster_names = self.browser.find_elements_by_xpath("//a[@data-hovercard-referer]")
        poster_names = self.browser.find_elements_by_xpath("//span/span/a[@aria-label]")
 

        #print("Procesar post", posts)
        ...
        for count, post in enumerate(posts):
            ...
            val = poster_names[count].get_attribute("aria-label")
            analysis = [val]  #poster_names[count].text
    ...
    def login(self, email, password):
        try:
            ...
            # clicking on login button u_0_b
